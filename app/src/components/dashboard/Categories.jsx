  ...
  return (
    <>
      ...
      <div
        ...
        className="mr-2 flex overflow-x-auto pl-4">
        {categories?.map((v, index) => {
          return (
            <div key={index} data-category={v.name} className="py-[1px]">
              <div
                onClick={() => handleClick(v.name)}
                className={`mr-2 box-border flex min-w-[80px] cursor-pointer flex-col items-center justify-center gap-2 rounded-md px-2 py-2 ${selected === v.name && "rounded-md border-primary bg-primaryBg outline outline-[1px] outline-primary"}`}>
                <img
                  src={v.image}
